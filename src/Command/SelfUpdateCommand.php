...
/*
 * This file is part of the Site Scanner package.
 *
...
namespace Defr\SiteScanner\Command;
...
class SelfUpdateCommand extends Command
{
    ...
    private function installerIsUpdated()
    {
        ...
        if (false === $remoteVersion = @file_get_contents(Application::VERSIONS_URL)) {
            throw new \RuntimeException('The new version of the Site Scanner couldn\'t be downloaded from the server.');
        }

        if (version_compare($localVersion, $remoteVersion, '>=')) {
            $this->output->writeln('<info>Site Scanner is already up to date.</info>');
            $isUpdated = true;
        } else {
            $this->output->writeln(sprintf('// <info>updating</info> Site Scanner to <comment>%s</comment> version', $remoteVersion));
        }
    ...
    private function downloadNewVersion()
    {
        ...
        if (!is_writable($this->currentInstallerFile)) {
            throw new \RuntimeException('Site Scanner update failed: the "' . $this->currentInstallerFile . '" file could not be written');
        }

        if (!is_writable($this->tempDir)) {
            throw new \RuntimeException('Site Scanner update failed: the "' . $this->tempDir . '" directory used to download files temporarily could not be written');
        }
    ...
    private function rollback()
    {
        $this->output->writeln(array(
            ...
            'The previous Site Scanner version has been restored.',
            '',
        ));
